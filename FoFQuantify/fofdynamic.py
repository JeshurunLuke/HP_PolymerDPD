...
def save(i):
    i = [str(j) for j in i]
    file_object = open('sampleG1S3.txt', 'a')
...
def itergroup(groupedP, groupedN, polymer, dim ):
    ...
    kappa = []
    for ROT in ROG2: 
        RGi = np.sqrt(np.sum(ROT))
        RG.append(RGi)
        bi = ROT[2] - (ROT[1]+ROT[0])/2
        b.append(bi)
        ci = ROT[1] -ROT[0]
        c.append(ci)
        kappa.append((bi**2 + (3/4)*ci**2)/(RGi**4))
    print(f"Part2 Radius of Gyration: {np.average(RG)}")

    
    return np.average(RG), np.average(b), np.average(c), np.average(kappa), np.average(Naggr), len(Naggr)
def graph(coords,i):
...
def gyrationTensor(coords):
    ...
    w, v = LA.eig(S)
    return np.sqrt(np.sum(w)), np.sort(w, axis=None)
...
def read_dumpBond(fname, analyze):
...
def evaluate(received):
    print(f"FOFing: {received[0]} ")

    df = received[1]
    polymer = isolate_polymer(df, polymeratomtype)
    groupedP = fof(polymer, dim)
    groupedN = fofN(polymer, dim)
    RG,  b, c, kappa, Naggr, GroupNumber = itergroup(groupedP, groupedN, polymer, dim)
    data = [ received[0],  RG,  b, c, kappa, Naggr, GroupNumber]
    save(data)
...
if __name__ == "__main__":
    ...
    finish = 625000
    iters = 20000
    analyze = np.arange(start, finish, iters)
    print(analyze)
    received = read_dumpBond(file, analyze)
    DOT = []
    count = 15
    ...
    processes = len(received)
    with Pool(processes) as p:
        J = p.starmap(evaluate, zip(received))
